...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class Advection extends ShaderPass {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      updatePass(args: any) {
        const { dt: d, isBounce: ib, BFECC: bf } = (args || {});
        if (!this.uniforms) return;
        if (typeof d === 'number') this.uniforms.dt.value = d;
        if (typeof ib === 'boolean') this.line.visible = ib;
        if (typeof bf === 'boolean') this.uniforms.isBFECC.value = bf;
        super.update();
      }
    }
    ...
    class ExternalForce extends ShaderPass {
      ...
      updatePass(props: any) {
        ...
        const cs = props.cursor_size || 0;
        const csX = cs * cellScale.x;
        const csY = cs * cellScale.y;
        const centerX = Math.min(Math.max(Mouse.coords.x, -1 + csX + cellScale.x * 2), 1 - csX - cellScale.x * 2);
        const centerY = Math.min(Math.max(Mouse.coords.y, -1 + csY + cellScale.y * 2), 1 - csY - cellScale.y * 2);
    ...
    class Viscous extends ShaderPass {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      updatePass(args: any) {
        const { viscous: visc, iterations, dt: d } = (args || {});
        if (!this.uniforms) return;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        let fbo_out: any;
        if (typeof visc === 'number') this.uniforms.v.value = visc;
    ...
    class Pressure extends ShaderPass {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      updatePass(args: any) {
        const { vel, pressure: p } = (args || {});
        if (this.uniforms && vel && p) { this.uniforms.velocity.value = vel.texture; this.uniforms.pressure.value = p.texture; }
        super.update();
      }
    }
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      private _frameInterval = 1000 / 30; // Cap at 30 FPS
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      constructor(props: any) {
        ...
        this._resize = () => { if (resizeTimer) return; resizeTimer = window.setTimeout(() => { resizeTimer = null; Common.resize(); this.output.resize(); }, 200); };
      ...
      dispose() {
        try {
          ...
          Mouse.dispose();
          if (Common.renderer) { const canvas = Common.renderer.domElement; if (canvas?.parentNode) canvas.parentNode.removeChild(canvas); Common.renderer.dispose(); Common.renderer.forceContextLoss(); }
        } catch { /* noop */ }
      }
    }
  ...
  return (
    <div
      ...
      style={{ pointerEvents: 'auto', touchAction: 'none', ...style }}
    />
  );
}
